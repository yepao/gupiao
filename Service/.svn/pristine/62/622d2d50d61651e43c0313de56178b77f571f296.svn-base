...
public class StockService {
	...
	public int addProfit_job(Map map,Date date) throws Exception
	{
		...
		try
		{
			...
			//stockDao.insertStockProfitByList(insertlist);
			//stockDao.insertStockSnapByList(insertlist1);
			...
			{
				StockSnap stockprofit=insertlist1.get(i);
				stockDao.insertStockSnap(stockprofit);
			}
	...
	public Map selectProfit(Map map) throws Exception
	{
		...
		try{
			...
			model.setUid((Integer)map.get("uid"));
			model.setCtype(1);
			...
			System.out.println(datelist.size()+""+date);
	...
	public Map getStockData(String stockids) throws Exception {
		...
		for(int i=0;i<beans.length;i++)
		{
			...
			if(temp.indexOf("v_hk")>-1)//港股
			{
				...
				tempdate.setTotalMarket(Double.parseDouble(data[45])*Math.pow(10, 8)+"");
